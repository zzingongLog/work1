...
int dy[] = {1,-1,0,0};
const int LIMIT = 10;
vector<int> gen(int k) { //4진법 변환
    vector<int> a(LIMIT);//k를 길이가 10인 4진법수로 변환한다
...
pair<bool,bool> simulate(vector<string> &a, int k, int &x, int &y) {
    if (a[x][y] == '.') return make_pair(false, false);//구슬이 빠진 후에 구슬 위치에 빈칸이 들어가 있는 상황 
    int n = a.size();
    int m = a[0].size();
    bool moved = false;
    while (true) {
        int nx = x+dx[k];
        int ny = y+dy[k];
        if (nx < 0 || nx >= n || ny < 0 || ny >= m) { //이부분은 필요없음
...
int main() {
    ...
    int ans = -1;
    for (int k=0; k<(1<<(LIMIT*2)); k++) { //전체 방법의 수는 4^10 = 2^20
        vector<int> dir = gen(k); //크기가 10인 4진법 수를 만들기
        if (!valid(dir)) continue; //의미가 없는 방법을 빼기
        int cur = check(a, dir);
        if (cur == -1) continue;
